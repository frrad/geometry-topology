...
\section{Explain in detail from the viewpoint of transversality theory, why the sum of the indices
  of a vector field with isolated zeroes on a compact orientable manifold $M$ is independent of what
  vector field we choose.}
\label{transv}

\section{Call the index sum in problem \ref{transv} the Euler characteristic $\chi(M)$. Explain why
...
\section{Suppose that $M$ is a triangulated compact orientable manifold, i.e., a manifold $M$
  represented as a finite simplicial complex.}

\subsection{Show that the alternating sum of the Betti numbers $b_0 - b_1 + b_2- \cdots$ (where $b_k
...
\subsection{Show that there is a vector field with the sum of its indices equal to the number
...
\section{Suppose $V$ is a smooth ($C^\infty$) vector field on $\R^3$ that is nonzero at
...
\section{Compute the homology of the real projective space \RP{n} for each $n > 1$.}

\advsection{}

\subsection{Define complex projective space \CP{n} $(n = 1,2,3, \dots)$}

\subsection{Show that \CP{n} is compact for all $n$.}

\subsection{Show that \CP{n} has a cell decomposition with one cell in each dimension
...
\section{Let $M = \RP{2} \times \RP{2}$ where \RP{2} is a real projective
